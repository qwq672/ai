...
class SettingsActivity : AppCompatActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            // ---------- 存储位置 ----------
            findPreference<ListPreference>(Settings.KEY_STORAGE_LOCATION)?.apply {
                summary = entry
                setOnPreferenceChangeListener { pref, newValue ->
                    val newLocation = newValue.toString()
                    val oldLocation = Settings.storageLocation
                    if (newLocation == oldLocation) return@setOnPreferenceChangeListener true
                    // 先切换 Settings，让 ModelStorage.modelDir 指向新位置
                    Settings.storageLocation = newLocation
                    val idx = findIndexOfValue(newLocation)
                    pref.summary = entries[idx]
                    AppLogger.i("Settings", "storage_location: $oldLocation -> $newLocation")
                    // 询问是否迁移
                    confirmStorageMigration(oldLocation, newLocation)
                    true
                }
            }

            // ---------- 界面 ----------
            findPreference<ListPreference>(Settings.KEY_DARK_MODE)?.apply {
                summary = entry
                setOnPreferenceChangeListener { pref, newValue ->
                    val idx = findIndexOfValue(newValue.toString())
                    pref.summary = entries[idx]
                    Settings.darkMode = newValue.toString()
                    // 立即应用，所有 Activity 自动 recreate
        ...
        private fun confirmStorageMigration(oldLocation: String, newLocation: String) {
            val ctx = requireContext().applicationContext
            // 计算旧位置中的文件
            val oldDir = if (oldLocation == "external") {
                java.io.File(ctx.getExternalFilesDir(null), "local_models")
            } else {
                java.io.File(ctx.filesDir, "local_models")
            }
            if (!oldDir.exists()) {
                Toast
                    .makeText(
                        requireContext(),
                        getString(R.string.storage_changed, newLocation),
                        Toast.LENGTH_SHORT,
                    )
                    .show()
                return
            }
            val files = oldDir.walkBottomUp().filter { it.isFile }.toList()
            if (files.isEmpty()) {
                Toast
                    .makeText(requireContext(), R.string.storage_migrate_none, Toast.LENGTH_SHORT)
                    .show()
                return
            }
            val totalBytes = files.sumOf { it.length() }
            val humanSize = human(totalBytes)
            AlertDialog
                .Builder(requireContext())
                .setTitle(R.string.storage_migrate_title)
                .setMessage(getString(R.string.storage_migrate_msg, files.size, humanSize))
                .setNegativeButton(R.string.storage_migrate_skip) { _, _ ->
                    Toast
                        .makeText(
                            requireContext(),
                            getString(R.string.storage_changed, newLocation),
                ...
                .setPositiveButton(R.string.storage_migrate_migrate) { _, _ ->
                    // 后台执行迁移，避免阻塞 UI
                    lifecycleScope.launch {
                        val (count, bytes) = withContext(Dispatchers.IO) {
                            com.geniex.demo.utils.ModelStorage.migrate(ctx)
                        }
                        Toast
                            .makeText(
                                requireContext(),
                                getString(R.string.storage_migrate_done, count, human(bytes)),
                                Toast.LENGTH_LONG,
                            )
                            .show()
                        AppLogger.i("Settings", "migrated: $count files, ${bytes} bytes")
                    }
                }
                .show()
        }
